...
exports.NotFoundError = exports.ConflictError = exports.ValidationError = void 0;
exports.analyzeAndCreateString = analyzeAndCreateString;
exports.getStringByValue = getStringByValue;
exports.getAllStringsWithFilters = getAllStringsWithFilters;
exports.deleteString = deleteString;
exports.parseNaturalLanguageQuery = parseNaturalLanguageQuery;
exports.getStringsByNaturalLanguage = getStringsByNaturalLanguage;
const crypto_1 = __importDefault(require("crypto"));
const stage1_model_1 = require("../model/stage1.model");
...
function frequencyOfCharacters(value) {
    const frequencyHash = {};
...
function lengthOfCharacters(value) {
    return value.length;
}
...
async function analyzeAndCreateString(value) {
    ...
    const existing = await stage1_model_1.AnalyzedString.findOne({ "properties.sha256_hash": sha256_hash });
    if (existing) {
    ...
    const newString = new stage1_model_1.AnalyzedString({
        value,
...
async function getStringByValue(value) {
    const sha256_hash = generateSha256(value);
    const result = await stage1_model_1.AnalyzedString.findOne({ "properties.sha256_hash": sha256_hash });
    if (!result) {
        throw new NotFoundError(`No analyzed string found for value: "${value}"`);
    }
    return result;
}
async function getAllStringsWithFilters(filters) {
    const query = {};
    ...
    const results = await stage1_model_1.AnalyzedString.find(query).sort({ created_at: -1 });
    return results;
}
async function deleteString(value) {
    const sha256_hash = generateSha256(value);
    const deleted = await stage1_model_1.AnalyzedString.findOneAndDelete({ "properties.sha256_hash": sha256_hash });
    if (!deleted) {
...
async function getStringsByNaturalLanguage(query) {
